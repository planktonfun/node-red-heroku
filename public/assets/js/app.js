...
$(document).ready(function($) {
	...
	$('a.dropdown-toggle.nav-link').on('click', () => {
        $('#notifications-count').hide();
        $.get('/reset-notifications', () => {});
        getFirstFiveActivities();
	})
...
function getRecentTime(timeStamp) {
...
function addMiniActivity(message, timeStamp) {
    let a = `
        ...
        <a href="activities">
            <div class="media">
                ...
                <div class="media-body">
                    ...
                    <p class="noti-time"><span class="notification-time">${getRecentTime(timeStamp)}</span></p>
                </div>
            </div>
        </a>
    </li>
    ...
    $('.notification-list').append(a);
}

function addMiniAttendance(rfid, timestamp) {
    let message = '';

    if(employeeIdsss[rfid] != undefined) {
        message = `${employeeIdsss[rfid].first} ${employeeIdsss[rfid].last} just checked in`;
    } else {
        message = `${rfid} not recognized. Please register`;
    }

    addMiniActivity(message, timestamp);
}

function getFirstFiveActivities() {

	$('.notification-list').html('');

    // $('.activity-list').html('');
    $.get('/get-employees', (res)=> {
        employeeIdsss = res.employeeIds;

        $.get('/get-activities', (res) => {
            let activities = res.activities.reverse();
            let minLength = activities.length;
            ...
            for (var i = 0; i < minLength; i++) {
                ...
                let rfid = activity[2];
                let timestamp = activity[3]*1;

                if(type == 'login')
                    addMiniAttendance(rfid, timestamp);
            }
